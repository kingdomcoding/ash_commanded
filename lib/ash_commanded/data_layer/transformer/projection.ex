defmodule AshCommanded.DataLayer.Transformer.Projection do
  ...
  def transform(%{persist: %{module: module}} = dsl_state) do
    ...
    api = Info.api(dsl_state)
    actions = Ash.Resource.Info.actions(dsl_state)

    module
    |> create_projectors(projections, actions, application, api)
    |> create_supervisor(module)

    {:ok, dsl_state}
  end

  def create_projectors(module, projections, actions, application, api) do
    projections
  ...
  def create_projectors(module, {projection, actions}, application, api) do
    actions
    |> Enum.map(fn action ->
      create_projector(module, projection, action, application, api)
    end)
  end

  def create_projector(module, projection, action, application, api) do
